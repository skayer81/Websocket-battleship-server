...
export class SinglPlayShips {
    ...
    constructor(fieldSize: number = 10) {
        ...
        this.field = Array.from({ length: fieldSize }, () => Array(fieldSize).fill(0));
        this.ships = [];
    }
    ...
    public randomizeShips(): void {
        ...
        const shipConfigurations: Ship[] = [
            { position: { x: 0, y: 0 }, direction: true, length: 4, type: "huge" , shots: []}, 
            { position: { x: 0, y: 0 }, direction: true, length: 3, type: "large"  , shots: []}, 
            { position: { x: 0, y: 0 }, direction: true, length: 2, type: "medium"  , shots: []}, 
            { position: { x: 0, y: 0 }, direction: true, length: 1, type: "small"  , shots: []}, 
        ];
        ...
        for (const shipType of shipConfigurations) {
            for (let count = 0; count < shipCounts[shipType.type]; count++) {
                let placed = false;
